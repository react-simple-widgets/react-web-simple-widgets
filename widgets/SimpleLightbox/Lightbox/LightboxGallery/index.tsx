...
import ProgressBarComponent from "./Container/ProgressBar";
import { LightboxCtx } from "../../context";
import { useInterval } from "../../hooks";
import { useIdle } from "../../hooks/useIdle";
import { useDebouncedCallback } from "use-debounce";
import subscribe from "subscribe-event";
import { HANDLE_ELEMENT, CLOSE_LIGHTBOX } from "../../context/actions";
...
const LightboxGallery = ({
    ...
    const [hideThumbnails, setHideThumbnails] = React.useState(false);

    // Check if the user is not taking any action
    const isIdle = useIdle(
        typeof settings.hideControlsAfter === "boolean" || settings.hideControlsAfter < 1000 ? 9999999 : settings.hideControlsAfter
    );

    // Method to get the index of a slide
    const getElementIndex = React.useCallback(
        (id) => {
    ...
    const establishNextOrPrevious = React.useCallback(
    ...
    // Handle Thumbnails
    const handleThumbnails = React.useCallback(() => {
        setHideThumbnails(!hideThumbnails);
    }, [hideThumbnails]);

    // Handle Panzoom
    const handlePanzoom = React.useCallback(
        (value) => {
    ...
    const setElementAndDirection = React.useCallback(
    ...
    const handleCurrentElement = React.useCallback(
        (elementID, currentID) => {
    ...
    function handleOnActive() {
        if (stageRef.current !== null && stageRef.current !== undefined) {
            if (stageRef.current.classList.contains("SRLIdle")) {
                stageRef.current.classList.remove("SRLIdle");
            }
        }
    }
    ...
    function handleOnIdle() {
        if (stageRef.current !== null && stageRef.current !== undefined) {
            stageRef.current.classList.add("SRLIdle");
        }
    }
    ...
    return (
        <div ref={stageRef} className="SRLStage">
